...
import com.android.example.cameraxapp.LumaListener
import com.android.example.cameraxapp.R
import com.android.example.cameraxapp.databinding.ActivityCameraxBinding
...
class CameraXActivity : AppCompatActivity() {
    ...
    private var lensFacing: Int = CameraSelector.LENS_FACING_BACK // 默认使用后置摄像头
    ...
    // 分析相机捕获的图像数据，以计算并返回图像的亮度（luminance）值
    private class LuminosityAnalyzer(private val listener: LumaListener) : ImageAnalysis.Analyzer {

        private fun ByteBuffer.toByteArray(): ByteArray {
            rewind()    // Rewind the buffer to zero 将缓冲区的当前位置重置为零，以便从头开始读取数据
            val data = ByteArray(remaining()) //返回当前缓冲区中剩余的字节数来创建 ByteArray
        ...
        override fun analyze(image: ImageProxy) {
            ...
            val luma = pixels.average() //计算所有像素的平均值，得到图像的亮度值
